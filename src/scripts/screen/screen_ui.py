...
class MarioKartUI:
    def __init__(self, root):
        ...
        ttk.Label(self.frm, text=f"Position: {POS}", font=("Helvetica", 28)).grid(column=4, row=1)
    ...
    def load_images(self):
        ...
        self.kai_tk = load_image(self.characters_path + "kai_toad_avi.png", (200, 200))
    ...
    def animate_item_selection(self):
        def update_image(index=0):
            if index < 15:  # Limit the animation to 10 cycles
                ...
                self.item_label.configure(image=random.choice(self.item_images))  # Final selection, add weights=WEIGHTS[POS]
